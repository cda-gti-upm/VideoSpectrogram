# Scan
# https://github.com/obspy/obspy/blob/master/obspy/imaging/scripts/scan.py
# !/usr/bin/env python
...
def parse_file_to_dict(data_dict, samp_int_dict, file, counter, format=None,
    ...
    if ignore_links and os.path.islink(file):
        ...
        return counter
    try:
        stream = read(file, format=format, headonly=False)
    except Exception:
        if verbose:
...
def add_stream_to_dict(data_dict, samp_int_dict, stream, verbose=False):
    ...
    for tr in stream:
        try:
            delta = 1. / (24 * 3600 * tr.stats.sampling_rate)  # sampling rate regarding a day instead of a second
        except ZeroDivisionError:
            if verbose:
                print("Skipping trace with zero sampling rate: {!s}".format(tr))
            continue
        _id = tr.get_id()
...
class Scanner(object):
    ...
    def parse(self, path, recursive=None, ignore_links=None):
        ...
        self.counter = parse_func(
            ...
            verbose=self.verbose, ignore_links=ignore_links)
