...
"""
Modifications:
- The data input are now pickle files containing obspy streams.
  - Read, sort, merge, sensor correction, 50Hz filtering, etc.
- An audio file is generated along the video file.
"""
...
def sonify_input(
    ...
    Args:
        ...
        format_in: format of data files
    ...
    st.sort(['starttime'])
    print(f'Data spans from {st[0].stats.starttime.strftime("%d-%b-%Y at %H:%M:%S")} until '
          f'{st[-1].stats.endtime.strftime("%d-%b-%Y at %H:%M:%S")}')
...
def _ffmpeg_combine(audio_file, video_file, output_file, call_str):
    ...
    args = [
        ...
        '2',
        '-metadata',
        f'artist= sonify_input',  # f'artist=sonify, rev. {__version__}',
        '-metadata',
        f'comment={call_str}',
        output_file,
    ]
...
def main():
    ...
    parser.add_argument(
        ...
        '--version',
        action='version',
        version=f'{parser.prog}',  # version=f'{parser.prog}, rev. {__version__}',
